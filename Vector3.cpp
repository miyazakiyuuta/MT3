...
void Vector3Math::VectorScreenPrintf(int x, int y, const Vector3& vector, const char* label) {
	Novice::ScreenPrintf(x, y, "%.02f", vector.x);
	Novice::ScreenPrintf(x + 60, y, "%.02f", vector.y);
	Novice::ScreenPrintf(x + 60 * 2, y, "%.02f", vector.z);
	Novice::ScreenPrintf(x + 60 * 3, y, "%s", label);
}
